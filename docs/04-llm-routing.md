...
4. **Shahriar configured the Manifest Router himself.** When it was time to set up the 5-tier routing, Shahriar applied the configuration directly without waiting for the agent. He knows his stack and makes decisions fast — the router was operational in minutes, not hours.
5. **Free tier for simple tasks is intentional architecture, not just cost-cutting.** ~60-70% of messages are simple (greetings, single-tool calls, clarifications). Burning reasoning tokens on these is wasteful. The free tier (GPT-5.4-mini via Copilot) handles them perfectly, and the router promotes to paid models seamlessly when complexity rises.
