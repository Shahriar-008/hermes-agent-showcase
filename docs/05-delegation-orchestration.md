...
5. **Background tee+tail logging is the pattern for long Claude Code sessions.** For multi-file changes that run for minutes, the prompt-file is piped to Claude Code via `terminal(background=True)` with output tee'd to a log file. Checking the log with `tail -f` during execution gives visibility without blocking the conversation. The Bettrix 8-fix session used exactly this pattern.
6. **Max spawn depth is 1 because grandchildren create chaos.** If a subagent could spawn its own subagents, error propagation and debugging become exponentially harder. Depth 1 means every child is a leaf — it does its work and returns. Simple, predictable, debuggable.
