...
6. **Three guardrail approaches were tried before landing on the current system.** First attempt: reasoning degradation on watchdog alert — created a death spiral (worse reasoning → more mistakes → more alerts). Second attempt: hard guardrail blocking in `tool_guardrails.py` — the guardrail blocked itself when it needed to make tool calls, requiring manual user intervention. Third attempt (current): SOUL.md pre-task gate rules + external watchdog alerts. The key insight: the agent needs feedback it can respond to, not handcuffs it has to fight.
7. **Each rejected approach taught something permanent.** Reasoning degradation proved negative feedback loops are real and dangerous. Hard blocking proved self-referential enforcement is a single point of failure. The current approach (SOUL.md + watchdog + audit) treats the agent like an engineer who needs feedback, not a process that needs a kill switch.
